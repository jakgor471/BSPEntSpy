	...
	public void refresh() {
		...
		for(int y = 0; y < raster.getHeight(); ++y) {
			for(int x = 0; x < raster.getWidth(); ++x) {
				...
				pixel2[0] = (int)Math.min(Math.max(pixel[0] * brightness + exposure, 0), 255);
				pixel2[1] = (int)Math.min(Math.max(pixel[1] * brightness + exposure, 0), 255);
				pixel2[2] = (int)Math.min(Math.max(pixel[2] * brightness + exposure, 0), 255);
