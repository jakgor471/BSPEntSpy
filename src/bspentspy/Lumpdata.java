...
class Lumpdata extends AbstractTableModel {
	...
	public void getData() {
		...
		for (int i2 = 0; i2 < list.size() - 1; ++i2) {
			int gap = ((Lumpentry) list.get((int) (i2 + 1))).offset
					- (((Lumpentry) list.get((int) i2)).offset + ((Lumpentry) list.get((int) i2)).length);
			if (gap <= 4)
				continue;
			temp.add(new Lumpentry("*", "Gap",
					((Lumpentry) list.get((int) i2)).offset + ((Lumpentry) list.get((int) i2)).length, gap));
		}
		list.addAll(temp);
		Collections.sort(list);
	...
	public String strint(int in) {
		char[] c = new char[] { (char) (in & 255), (char) (in >> 8 & 255), (char) (in >> 16 & 255),
				(char) (in >>> 24 & 255) };
		return new String(c);
	}
}
