...
public class FGD {
	...
	public String getClassHelp(String classname) {
		...
		for (Property e : entry.properties) {
			if (e.name.equals("spawnflags"))
				continue;
			sb.append("<p><b>").append(e.getDisplayName()).append("</b> <i>").append(e.name).append("</i> &lt;")
			...
			if (e.type == DataType.choices) {
				...
				for (PropChoicePair ch : choices) {
					sb.append("<li>").append(ch.value).append(" : ").append(sanitiseText(ch.description))
							.append("</li>");
				}
				sb.append("</ul>");
			}
		...
		if (entry.fgdDefinedIndex > -1 && entry.fgdDefinedIndex < loadedFgds.size()) {
			...
			sb.append("<p><i>Defined in '").append(filename).append("' at line ").append(entry.fgdDefinedLine)
					.append("</i><p>");
		}
			...
			if (tok.isIdent()) {
				if (tok.value.equals("base")) {
					...
					if (lexer.getToken().isOpenParen()) {
						lexer.consume();
						while (lexer.getToken() != null && !lexer.getToken().isCloseParen()) {
							lexer.consume();
						}
						lexer.expect(BasicTokenType.symbol, ")");
						lexer.consume();
	...
	private static FGDEntry parseClass(FGDLexer lexer, FGD fgdData, int fileIndex) throws LexerException {
		...
		if (!FGDEntry.isValidClass(lexer.expect(BasicTokenType.ident).value))
			throw new LexerException(lexer, "Unknown class type '" + lexer.getToken().value + "'!");
		...
		if (baseClasses != null) {
			...
			for (String s : baseClasses) {
				if (!fgdData.classMap.containsKey(s))
					throw new LexerException(lexer,
							"Class '" + newClass.classname + "' derives from not existing '" + s + "'!!!");
				...
				for (Property p : base.properties)
					newClass.addProperty((Property) p.copy()); // copy for inheriting spawnflags and not messing
																// everything up
	...
	private static class FGDToken extends BasicToken {
		...
		public boolean isNumericOrSign() {
			return type == BasicTokenType.numeric
					|| ((value.equals("-") || value.equals("+")) && type == BasicTokenType.symbol);
		}
	...
	private class FGDLexer extends Lexer<FGDToken> {
		...
		public FGDToken expect(BasicTokenType type) throws LexerException {
			...
			if (currToken.type != type)
				throw new LexerException(this, "Unexpected token type. Expected '" + type.toString() + "', got '"
						+ currToken.type.toString() + "'!");

			return (FGDToken) currToken;
		}

		// throws exception if token is null or token is not of a specified type or it's
		// value differs from that specified
		public FGDToken expect(BasicTokenType type, String value) throws LexerException {
			...
			if (currToken.type != type || !currToken.value.equals(value))
				throw new LexerException(this,
						"Unexpected token. Expected '" + value.toString() + "', got '" + currToken.value + "'!");
